...
y1 <- c(rep(rep(1,nrow(data)), data[,3]),
        rep(rep(2,nrow(data)), data[,4]),
        rep(rep(1,nrow(data)), data[,5]))
y2 <- c(rep(rep(1,nrow(data)), data[,3]),
        rep(rep(2,nrow(data)), data[,4]),
        rep(rep(2,nrow(data)), data[,5]))
menopause <- cbind(x1,x2,y1,y2)
dimnames(menopause) <- list(NULL, c("x1","x2","y1","y2"))
